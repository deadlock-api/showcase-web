...
export const useAnalysisStore = create<AnalysisState>((set, get) => ({
  ...
  addExcludedItem: (itemId) => set((state) => ({
    excludedItems: [...state.excludedItems, itemId],
  })),

  removeExcludedItem: (itemId) => set((state) => ({
    excludedItems: state.excludedItems.filter((id) => id !== itemId),
  })),

  removeManyExcludedItems: (itemIds) => set((state) => ({
  ...
  submitAnalysis: async () => {
    ...
    try {
      ...
      const result = await fetch(`https://analytics.deadlock-api.com/v1/dev/win-rate-analysis?${queryParams.toString()}`, {
        method: "POST",
        headers: {
      ...
      set({
        analysisResult: {
          winRate: filteredData.map((item) => ({ item: Item.byId(item.item_id), winRate: item.win_rate, sampleSize: item.total })),
          pickRate: [],
        },
      });
    } catch (e) {
